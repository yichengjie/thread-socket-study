...
import com.yicj.service.impl.UserServiceImpl;
import javassist.*;
...
public class JavassistHello {
    ...
    public static void main(String[] args) throws Exception {
        ...
        //引用Service，UserServiceImpl
        clsPool.importPackage(UserService.class.getPackage().getName());
        //
        //
        //拼装类全名，动态创建类
        CtClass ctCls = clsPool.makeClass(String.format("%s.%s",pkgName,clsName)) ;
        //为这个类添加要实现的接口
        ctCls.addInterface(clsPool.getCtClass(UserService.class.getName()));
        //
        //
        //添加一个字段
        CtField svcField = CtField.make("private UserService service ; \n", ctCls) ;
        ...
        //
        //添加构造函数
        CtConstructor ctConstructor = new CtConstructor(new CtClass[]{clsPool.getCtClass(UserService.class.getName())}, ctCls) ;
        ...
        Object obj = clazz.getConstructor(UserService.class).newInstance(new UserServiceImpl());
        //通过反射调用上面的test方法
        obj.getClass().getMethod("test", new Class[]{}).invoke(obj) ;
        //也可以转为对应的接口，在调相应的接口方法
        ((UserService)obj).process(2);
    }
}
